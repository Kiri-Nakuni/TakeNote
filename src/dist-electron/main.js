...
function createWindow() {
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    mainWindow.loadFile(path.join(__dirname, "../src/renderer/mode/general/index.html"));
  }
...
function initializeWindowEvents() {
  electron.ipcMain.on("update-title", (event, filePath) => {
    if (mainWindow) {
      if (filePath) {
...
electron.ipcMain.on("log-error", (event, error) => {
  const logEntry = {
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
    type: "rendererError",
    error
...
electron.ipcMain.handle("export-logs", async () => {
  const date = /* @__PURE__ */ new Date();
  const yy = date.getFullYear().toString().slice(-2);
  const mm = (date.getMonth() + 1).toString().padStart(2, "0");
  const dd = date.getDate().toString().padStart(2, "0");
  const fileName = `${yy}${mm}${dd}.log`;
  const logFilePath = path.join(electron.app.getPath("userData"), fileName);
  try {
...
const notesDir = path.join(electron.app.getPath("userData"), "Notes");
...
electron.ipcMain.handle("get-files", async (event, subDir = "") => {
  try {
...
electron.ipcMain.handle("read-file", async (event, fileName) => {
  try {
    const filePath = resolveSecurePath(fileName);
    const fileBuffer = await fs.readFile(filePath);
    try {
      const zip = new AdmZip(fileBuffer);
      const zipEntry2 = zip.getEntry("index.html");
      if (zipEntry2) {
        return zipEntry2.getData().toString("utf-8");
      }
...
electron.ipcMain.handle("write-file", async (event, fileName, content) => {
  try {
    const filePath = resolveSecurePath(fileName);
...
electron.ipcMain.handle("create-directory", async (event, dirName) => {
  try {
